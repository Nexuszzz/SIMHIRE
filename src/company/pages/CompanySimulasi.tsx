...
import { 
  ...
  Search, Plus, X, Send, FileText, Calendar, Sparkles
} from 'lucide-react';
import { MOCK_SIMULASI_RESULTS, SIMULASI_CATEGORIES, getRankColor } from '@/lib/simulasiData';
...
const CompanySimulasi: React.FC = () => {
  ...
  const [selectedCandidate, setSelectedCandidate] = useState<typeof MOCK_SIMULASI_RESULTS[0] | null>(null);

  // Filter results
  const filteredResults = MOCK_SIMULASI_RESULTS.filter(result => {
  ...
  const handleViewDetail = (candidate: typeof MOCK_SIMULASI_RESULTS[0]) => {
    setSelectedCandidate(candidate);
    setShowDetailModal(true);
  };

  const handleInviteClick = (candidate: typeof MOCK_SIMULASI_RESULTS[0]) => {
    setSelectedCandidate(candidate);
  ...
  const avgScore = Math.round(filteredResults.reduce((sum, r) => sum + r.percentage, 0) / totalCandidates);
