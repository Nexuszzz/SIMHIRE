...
import {
  Clock, CheckCircle2, XCircle, AlertTriangle, ArrowRight, ArrowLeft,
  Trophy, Target, Zap, Code, FileText, Sparkles, Play, Pause, RotateCcw, Star,
  Upload, File, X as XIcon, Eye, Copy
} from 'lucide-react';
...
import { saveSimulasiResult, type SavedSimulasiResult } from '@/lib/storage';
...
const SimulasiExecution: React.FC = () => {
  ...
  // Initialize timer based on category duration
  useEffect(() => {
    if (category) {
      // Parse duration string (e.g., "2-3 jam" -> use 2 hours = 120 minutes)
  ...
  // Calculate score for current task (mock scoring)
  const calculateTaskScore = (answer: string, task: SimulasiTask): number => {
    const answerLength = answer.trim().length;
    
    // Base score on answer length and quality indicators
    let score = 0;
    ...
    // Quality indicators (up to 70 points)
    ...
    // Bonus for structure (paragraphs)
    const paragraphs = answer.split('\n\n').length;
    if (paragraphs > 1) score += 10;
    
    // Bonus for code blocks (if applicable)
    if (task.type === 'coding' && answer.includes('```')) score += 10;
    
    // Bonus for bullet points
    if (answer.includes('-') || answer.includes('*')) score += 10;
  ...
  const handleAutoSubmit = () => {
    ...
    // Submit current task if not done
  ...
  // Finish simulasi and save results
  const handleFinishSimulasi = () => {
    ...
    const percentage = Math.round((totalScore / (tasks.length * 100)) * 100);
    ...
    const result: SavedSimulasiResult = {
      ...
      percentage,
      breakdown,
      badge: percentage >= 85 ? category.badge : undefined,
      certificate: percentage >= 80 ? `CERT-${category.id.toUpperCase()}-${Date.now()}` : undefined,
    };

    // Save to localStorage
    saveSimulasiResult(result);

    // Navigate to results page
    navigate(`/dashboard/simulasi-kerja/${categoryId}/results`, {
      state: { result, submissions }
    });
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-primary-50/20 to-gray-50">
      ...
      <div className="max-w-5xl mx-auto px-6 py-8">
        <AnimatePresence mode="wait">
          ...
          >
            {/* Task Card - Enhanced with Gradient Border */}
