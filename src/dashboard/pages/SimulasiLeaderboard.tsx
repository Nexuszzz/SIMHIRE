...
import { loadSimulasiResults, type SavedSimulasiResult } from '@/lib/storage';
...
const SimulasiLeaderboard: React.FC = () => {
  ...
  // Load real results from localStorage
  useEffect(() => {
    const stored = loadSimulasiResults();
    // Combine with mock data for better demo
    const combined = [...stored, ...MOCK_SIMULASI_RESULTS];
    
    // Remove duplicates by categoryId (prefer real user data)
    const unique = combined.reduce((acc, curr) => {
      const existing = acc.find(r => r.categoryId === curr.categoryId && r.userId === curr.userId);
      if (!existing) {
        acc.push(curr);
      }
      return acc;
    }, [] as SavedSimulasiResult[]);
    
    setRealResults(unique);
  }, []);
